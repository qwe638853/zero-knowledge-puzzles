...
void NotEqual_1_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void NotEqual_1_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "NotEqual";
ctx->componentMemory[coffset].signalStart = soffset;
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 2]); // line circom 17
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
Fr_sub(&expaux[0],&circuitConstants[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0]); // line circom 19
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 1; i++){
...
NotEqual_1_create(1,0,ctx,"main",0);
NotEqual_1_run(0,ctx);
}
